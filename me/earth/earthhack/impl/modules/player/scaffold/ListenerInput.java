//Deobfuscated with https://github.com/SimplyProgrammer/Minecraft-Deobfuscator3000 using mappings "C:\Users\aesthetical\Documents\Development\Tools\Minecraft-Clients\1.12.2 mappings"!

// 
// Decompiled by Procyon v0.6-prerelease
// 

package me.earth.earthhack.impl.modules.player.scaffold;

import me.earth.earthhack.impl.event.listeners.*;
import me.earth.earthhack.impl.event.events.movement.*;

final class ListenerInput extends ModuleListener<Scaffold, MovementInputEvent>
{
    public ListenerInput(final Scaffold module) {
        super(module, (Class<? super Object>)MovementInputEvent.class);
    }
    
    public void invoke(final MovementInputEvent event) {
        if (((Scaffold)this.module).down.getValue() && ((Scaffold)this.module).fastSneak.getValue() && ListenerInput.mc.gameSettings.keyBindSneak.isKeyDown() && !ListenerInput.mc.gameSettings.keyBindJump.isKeyDown()) {
            event.getInput().sneak = false;
            event.setCancelled(true);
        }
    }
}
