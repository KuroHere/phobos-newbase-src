...
final class ListenerAttack extends ModuleListener<FakePlayer, PacketEvent.Send<CPacketUseEntity>>
{
    ...
    public ListenerAttack(final FakePlayer module) {
    ...
    public void invoke(final PacketEvent.Send<CPacketUseEntity> event) {
        ...
        final Entity entity = event.getPacket().getAttackedEntity();
        if (((FakePlayer)this.module).fakePlayer.equals(entity)) {
            event.setCancelled(true);
            if (ListenerAttack.CRITICALS.isEnabled() || (!ListenerAttack.mc.player.isSprinting() && ListenerAttack.mc.player.fallDistance > 0.0f && !ListenerAttack.mc.player.onGround && !ListenerAttack.mc.player.isOnLadder() && !ListenerAttack.mc.player.isInWater() && !ListenerAttack.mc.player.isPotionActive(MobEffects.BLINDNESS) && !ListenerAttack.mc.player.isRiding())) {
            ...
            else if (ListenerAttack.mc.player.getCooledAttackStrength(0.5f) > 0.9) {
    ...
    static {
        CRITICALS = Caches.getModule(Criticals.class);
    }
}
