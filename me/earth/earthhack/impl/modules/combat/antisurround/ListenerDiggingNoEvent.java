...
final class ListenerDiggingNoEvent extends ModuleListener<AntiSurround, PacketEvent.NoEvent<CPacketPlayerDigging>>
{
    ...
    public ListenerDiggingNoEvent(final AntiSurround module) {
        super(module, (Class<? super Object>)PacketEvent.NoEvent.class, -1000, CPacketPlayerDigging.class);
        this.function = new PreCrystalFunction(module);
    }
    
    public void invoke(final PacketEvent.NoEvent<CPacketPlayerDigging> event) {
        if (event.isCancelled() || !event.hasPost() || event.getPacket().getAction() != CPacketPlayerDigging.Action.STOP_DESTROY_BLOCK || ((AntiSurround)this.module).holdingCheck() || !((AntiSurround)this.module).preCrystal.getValue() || !event.getPacket().isClientSideBreaking()) {
            return;
        }
