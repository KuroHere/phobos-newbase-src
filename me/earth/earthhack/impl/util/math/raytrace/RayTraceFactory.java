...
public class RayTraceFactory implements Globals
{
    ...
    public static Ray fullTrace(final Entity from, final IBlockAccess world, final BlockPos pos, final double resolution) {
        ...
        for (final EnumFacing facing : getOptimalFacings(from, pos)) {
            ...
            if (!state.getMaterial().isReplaceable()) {
                ...
                final double dist = BlockUtil.getDistanceSq(from, offset);
    ...
    public static Ray rayTrace(final Entity from, final BlockPos on, final EnumFacing facing, final IBlockAccess access, final IBlockState state, final double res) {
        final Vec3d start = PositionUtil.getEyePos(from);
        ...
        if (res >= 1.0) {
            ...
            final Vec3d rotations = start.addVector(look.xCoord * d, look.yCoord * d, look.zCoord * d);
            final RayTraceResult result = RayTracer.trace((World)RayTraceFactory.mc.world, access, start, rotations, false, false, true);
            ...
            if (res <= 0.0) {
                ...
                for (final Vec3d vec : vectors) {
                    final RayTraceResult ray = RayTracer.trace((World)RayTraceFactory.mc.world, access, start, vec, false, false, true);
            ...
            for (double x2 = Math.min(minX, maxX); x2 <= endX; x2 += res) {
                for (double y2 = Math.min(minY, maxY); y2 <= endY; y2 += res) {
                    for (double z2 = Math.min(minZ, maxZ); z2 <= endZ; z2 += res) {
                        ...
                        final RayTraceResult ray2 = RayTracer.trace((World)RayTraceFactory.mc.world, access, start, vector, false, false, true);
    ...
    static float[] rots(final Entity from, final Vec3d vec3d) {
        return RotationUtil.getRotations(vec3d.xCoord, vec3d.yCoord, vec3d.zCoord, from);
    }
